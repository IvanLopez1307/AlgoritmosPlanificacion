...
public class Gui extends JFrame {
    ...
    public Gui() {
        ...
        int Numero_de_Procesosro = cantidad_Procesos;
        ...
        while (on) {
            ...
            if (!procesos.isEmpty()) {
                ...
                for (Nodo nodo : procesos) {
                    if (nodo.getTiempoEjecucion() <= mini) {
                        mini = nodo.getTiempoEjecucion();
                        pos = cont++;
                    } else {
                        cont++;
                    }
        ...
        // Alistar arraylists para las matricas
        FCFS0.clear();
        FCFS0.addAll(FCFS);
        for (Nodo n : FCFS) {
        ...
        int Ncol = Numero_de_Procesosro + 1;
        ...
        for (int fil = 0; fil < 9; fil++) {
            ...
            for (int col = 1; col < Ncol; col++, con++) {
                switch (fil) {
                    case 0 ->
                        datos[fil][col] = list.get(con).getHoraLlegada();
                    case 1 ->
                        datos[fil][col] = list.get(con).getTiempoEjecucion();
                    case 2 ->
                    ...
                    case 6 ->
                        datos[fil][col] = FCFS0.get(con).tiempoFinalizacion - FCFS.get(con).getHoraLlegada();
                    case 7 ->
                        datos[fil][col] = FCFS0.get(con).tiempoFinalizacion - FCFS.get(con).getHoraLlegada() - list.get(con).getTiempoEjecucion();
                    case 8 ->
                        datos[fil][col] = (FCFS0.get(con).tiempoFinalizacion - FCFS.get(con).getHoraLlegada()) / (float) list.get(con).getTiempoEjecucion();
                }
            }
        }
